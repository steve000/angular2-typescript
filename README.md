...
二、前端开发启动程序请见如下步骤，：
    1、环境准备：npm+yarn（见上述环境准备内容）
    2、在工程root 目录下，运行“yarn install” 安装包（见重要说明）；
    3、运行“yarn start”，即可启动
    4、如想了解更多关于整个工程架构内容，见四、微服务模块
...
1、服务注册器（如果在公司内部环境进行开发， 已用docker 启动了服务注册，可以直接使用该服务，以节省本地资源。前端开发人员可以跳过此步骤）
...
2、用户认证授权（注明：如果在公司内部环境进行开发， 已用docker 启动了uaa服务，可以直接使用该服务，以节省本地资源。前端开发人员可以跳过此步骤）
